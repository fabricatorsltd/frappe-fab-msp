...
2. **Classification.** On validate, `apply_service_rules` sets the ticket's
   approval status (Pending when the type requires approval, else Not Required)
   and billing status (Pending when billable, else Not Billable). Manager
   decisions and issued invoices are never overwritten.
3. **Service link.** For `Create` the agent provisions a Customer Service from
   the ticket ("Create service"); for `Modify` the agent links the existing one.
4. **Approval.** If required, the approver decides via Approve/Reject. Routing:
...
5. **Fulfillment** (on approval of a billable request, run as system):
